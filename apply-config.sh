...
if [ "$4" == "" ]; then
  ...
  VAULT_PASSWORD=$4 ansible-playbook -e input_file=${INPUT_PATH}/${INPUT_FILE} -e broker_host=${TARGET} ${PLAYBOOK_PATH}/${PLAYBOOK}_pb.yml --vault-password-file ./.vault_pass
fi